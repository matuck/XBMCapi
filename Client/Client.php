...
use matuck\XBMCapi\Client\Server;
...
use matuck\XBMCapi\Exception\XBMCCommandException;
...
use matuck\XBMCapi\Exception\XBMCResponseException;
use matuck\XBMCapi\Exception\XBMCServerException;
...
abstract class Client
{
    ...
    /**
     * @var XBMCNamespace The root namespace instance.
     * @access private
     */
    ...
    public function __construct($parameters) {
        ...
        } catch (XBMCServerException $e) {
            throw new XBMCConnectionException($e->getMessage(), $e->getCode(), $e);
        }
        $this->server = $server;
     ...
     * @exception XBMCInvalidCommandException if the called command does not
     ...
     * @return XBMCNamespace The requested namespace.
     * @exception XBMCInvalidNamespaceException if the namespace does not
     * exist in the root namespace.
     * @access public
     */
    ...
    private function createRootNamespace() {
        ...
        $this->rootNamespace = new XBMCNamespace('root', $commands, $this);
    }
     ...
     * @exception XBMCRequestException if it is not possible to retrieve a list of
     * available commands.
     * @access private
     */
    private function loadAvailableCommands() {
        try {
            ...
            throw new XBMCRequestException(
     ...
     * @exception XBMCRequestException if it was not possible to make the request.
     * @exception XBMCResponseException if the response was not being properly received.
     * @access private
     */
    private function sendRpc($command, $params = array()) {
        ...
        if (!$this->checkResponse($response, $rpcId)) {
            throw new XBMCResponseException('JSON RPC request/response ID mismatch');
        }
